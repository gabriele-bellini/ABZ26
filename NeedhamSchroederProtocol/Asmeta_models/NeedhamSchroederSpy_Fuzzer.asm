...
signature:	
	...
	controlled totTerminatedProtocols: Integer
	controlled totStartedProtocols: Integer
	...
	main rule r_Main = 
		...
		// do one transition in the simulated model
			if not(isRunTerminated) then 
				...
				par
					// save model behavior in global attributes for model analysis or to check hyperproperties
					totTerminatedProtocols := totTerminatedProtocols + terminatedSession
					totStartedProtocols := totStartedProtocols + startedSessions
...
default init s0:
	...
	function totTerminatedProtocols = 0
	function totStartedProtocols = 0
